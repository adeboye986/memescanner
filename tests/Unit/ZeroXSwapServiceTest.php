...
class ZeroXSwapServiceTest extends TestCase
{
    ...
    public function test_firm_native_eth_quote_is_normalized_and_validated(): void
    {
        ...
        Http::fake(['https://api.0x.test/swap/allowance-holder/quote*' => Http::response($this->response())]);

        $quote = app(ZeroXSwapService::class)->quote(self::WALLET, self::TOKEN, '1000000000000000', 100);
        ...
        Http::assertSent(fn ($request): bool => $request->hasHeader('0x-api-key', 'zero-x-secret')
            && $request->hasHeader('0x-version', 'v2')
            && $request->url() === 'https://api.0x.test/swap/allowance-holder/quote?chainId=1&sellToken='.ZeroXSwapService::NATIVE_ETH.'&buyToken='.self::TOKEN.'&sellAmount=1000000000000000&taker='.self::WALLET.'&slippageBps=100');
    }
